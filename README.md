...
它的目的不是幫你「加速任務」，而是幫你把任務**管清楚**：開始時先定義 checkpoint，執行中只回報可驗證事實，遇到卡住時立即升級，完成時附上驗證與交付資訊。這樣使用者不會只看到模糊的「還在跑」，而是能一眼知道現在做到哪、憑什麼這樣判斷、下一步是什麼。
...
1. **先建 task record，再開始做事**  
   一開始就定義 `task_id`、目標、workflow、預期產物。

2. **用 checkpoint 管流程，不用流水帳敘事**  
   任務進度以明確階段表示，而不是大量思考過程或模糊描述。

3. **只報可驗證事實（verifiable facts）**  
...
一句話：**只要任務會跑很久、會等待、會分段、會交接，這個 skill 就適合。**
