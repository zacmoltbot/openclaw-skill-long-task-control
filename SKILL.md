...
This skill is now a **semi-enforced task control system**, not just a formatting guide. The contract has six layers:
...
4. **Executor subagent** *(new)*: drives multi-stage tasks to completion by executing each step sequentially and writing checkpoints; auto-resumes from the current ledger step
5. **Low-cost monitor cron**: watches the executor; GAP-1 detects when a step is done but the next hasn't started; one-shot nudge then escalate; passive delivery push every tick
6. **Machine checks**: timeout detector + compliance checker
...
## Executor subagent *(new — the actual work engine)*

The executor is an OpenClaw subagent that **drives the task to completion**, not just monitors it.

**Architecture:**
- Monitor cron: watches the executor, sends GAP-1 nudge if a step is done but next hasn't started, passive delivery push
- Executor subagent: reads ledger → executes next step → writes checkpoint → repeats
- The monitor observes the executor, not the other way around

**When to use:**
- Any multi-stage task where steps should proceed automatically without waiting for the main agent to be prompted
- Pure in-process multi-stage work with no external jobs at all
...
This installs an executor cron that wakes every 5 minutes, reads the ledger's `current_checkpoint`, executes the next pending step, and writes a CHECKPOINT. The monitor cron runs alongside and handles GAP-1 / nudge / delivery push.

**Auto-resume:** When the executor starts (or restarts), it reads the ledger to find `current_checkpoint`. If step-01 is DONE and step-02 is RUNNING, it starts step-02 immediately — no rework.

**Executor prompt contract:**
The executor prompt (generated by `executor_prompt()` in `openclaw_ops.py`) tells the agent:
1. Run `executor_engine.py preview <task_id>` to see what to do next
2. Execute the next step (shell command / API call / file operation)
3. Write CHECKPOINT via `task_ledger.py checkpoint`
4. Loop until `workflow_complete` or `blocked`
5. Output `EXECUTOR_DONE` or `EXECUTOR_BLOCKED`

**Executor metadata:** `monitoring.executor_cron_job_id`, `monitoring.executor_session_key`, `monitoring.executor_state=ACTIVE|RUNNING|COMPLETED|BLOCKED`.
...
**Step-completion stall (GAP-1):** A new class of stall is detected when the current step's workflow sub-state is terminal (DONE/COMPLETED/FAILED/BLOCKED) but the task hasn't moved to the next step and no new checkpoint arrives within one check interval (5 minutes). This catches the case where step-01 completes (e.g. external job done, checkpoint written) but step-02 never starts — previously the monitor would say OK forever because `pending_external` flips False after the external job completes and `last_checkpoint_at` is recent.

**One-shot escalation (fixed):** With GAP-1 detection, the monitor fires `NUDGE_MAIN_AGENT` **once** on the first check after the step enters terminal state. On the **next** tick, if the same step is still stalled (step already nudged for GAP-1), the monitor immediately escalates to `BLOCKED_ESCALATE` — no repeat nudge, no 3-tick wait. The nudged-steps set is tracked in `monitoring.gap1_nudged_steps[]` and cleared when the step advances. This works even for tasks with no external jobs at all.
...
4. **被動 Delivery Push（重要）**：Monitor 每 5 分鐘 tick 時，會自動檢查 ledger 中 `reporting.pending_updates[]` 是否有 `delivered=false` 的項目。若有，**不需要等 main agent 主動來問**，monitor 會馬上用 `message.send` 推到 Discord channel。成功發送後，更新 ledger 的 `delivered=true`。這樣確保「你不來問，我就不報」的缺口被補起來。用 `preview-tick` 輸出中的 `pending_user_updates_deliverable_count` 和 `pending_user_updates_deliverable` 來追蹤未送出的更新。 Delivery push 在**每一次 tick 都先於狀態評估執行**，不受 notify flag 限制。

4a. **新 nudge delivery 架構**：Monitor 的 `cron_prompt`（由 `openclaw_ops.py cron_prompt()` 生成）現在使用 `sessions_send` 直接喚醒 owner agent，而不是發 Discord：
   - `NUDGE_MAIN_AGENT` → `sessions_send --sessionKey agent:main:discord:channel:1484432523781083197`，訊息內容：「Task X stalled at step Y. Resume / checkpoint / or report BLOCKED. Next action: Z」
   - `OWNER_RECONCILE` → 同上 sessions_send，訊息內容請 owner 回報 A/B/C/D/E branch
   - `BLOCKED_ESCALATE` / `COMPLETED` / milestone checkpoints → 才用 `message.send` 發 Discord 通知 user
   - 這樣確保 owner agent 被喚醒續行，不靠 Discord 訊息被動等待，避免重複 Discord nudge spam user
