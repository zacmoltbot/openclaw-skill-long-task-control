...
def normalize_workflow_step_title(value: str) -> str:
    text = " ".join((value or "").strip().split())
    if not text:
        raise SystemExit("Workflow step title cannot be empty")
    return text



def compile_validated_workflow(goal: str, workflow_steps):
    normalized = []
    seen = set()
    for idx, raw in enumerate(workflow_steps or [], start=1):
        title = normalize_workflow_step_title(raw)
        lower = title.lower()
        if any(pattern in lower for pattern in DANGEROUS_WORKFLOW_PATTERNS):
            raise SystemExit(f"Workflow rejected: step {idx} contains dangerous instruction: {title}")
        if any(pattern in lower for pattern in COARSE_WORKFLOW_PATTERNS):
            raise SystemExit(f"Workflow rejected: step {idx} is too coarse-grained: {title}")
        if title.count(" and ") + title.count(" then ") + title.count(" afterwards ") >= 2:
            raise SystemExit(f"Workflow rejected: step {idx} bundles multiple actions; split it first: {title}")
        if len(title.split()) > 14:
            raise SystemExit(f"Workflow rejected: step {idx} is too long / coarse; split it first: {title}")
        slug = lower
        if slug in seen:
            raise SystemExit(f"Workflow rejected: duplicate step title: {title}")
        seen.add(slug)
        normalized.append({
            "id": f"step-{idx:02d}",
            "title": title,
    ...
    if not normalized:
        raise SystemExit(f"Workflow rejected: task '{goal}' must provide an explicit recoverable workflow")
    return normalized
...
def ensure_task_shape(task):
    ...
    task["heartbeat"].setdefault("progress_idle_threshold_sec", task["heartbeat"].get("timeout_sec", 1800))
...
def project_task(task):
    ...
    for idx, step in enumerate(workflow):
        ...
        step_event_at = obs.get("completed_at") or obs.get("last_progress_at") or obs.get("blocked_at") or obs.get("failed_at")
        if latest_step_event_at is None:
            latest_step_event_at = step_event_at
        else:
            latest_step_event_at = max(filter(None, [latest_step_event_at, step_event_at]))
        if obs.get("completed_at"):
            progress_signal_candidates.append({"kind": "step_completed", "step_id": step_id, "at": obs.get("completed_at")})
        if obs.get("last_progress_at"):
            progress_signal_candidates.append({"kind": "step_progress", "step_id": step_id, "at": obs.get("last_progress_at")})
        if obs.get("blocked_at"):
            progress_signal_candidates.append({"kind": "step_blocked", "step_id": step_id, "at": obs.get("blocked_at")})
        if obs.get("failed_at"):
            progress_signal_candidates.append({"kind": "step_failed", "step_id": step_id, "at": obs.get("failed_at")})
    ...
    for key, job in task.get("observed", {}).get("external_jobs", {}).items():
        ...
        observed_at = job.get("observed_at") or job.get("updated_at")
        if observed_at:
            progress_signal_candidates.append({
                "kind": "external_job_status",
    ...
    for idx, step in enumerate(workflow_states):
        if step["state"] != "DONE":
            first_open_index = idx
            break
    if first_open_index is not None:
        for idx, step in enumerate(workflow_states[first_open_index + 1:], start=first_open_index + 1):
            if step["state"] == "DONE":
                inconsistencies.append(f"serial_out_of_bounds:{workflow_states[first_open_index]['id']}->done_after:{step['id']}")
            if step["state"] == "RUNNING":
                inconsistencies.append(f"serial_out_of_bounds:{workflow_states[first_open_index]['id']}->running_after:{step['id']}")
    ...
    if completed_task and completed_task.get("completed_at"):
        progress_signal_candidates.append({"kind": "task_completed", "at": completed_task.get("completed_at")})
    if blocked_task and blocked_task.get("blocked_at"):
        progress_signal_candidates.append({"kind": "task_blocked", "at": blocked_task.get("blocked_at")})

    executor_health = task.get("monitoring", {}).get("executor_health", {}) or {}
    if executor_health.get("last_success_at"):
        progress_signal_candidates.append({
            "kind": "executor_healthy",
            "at": executor_health.get("last_success_at"),
            "state": executor_health.get("state"),
            "consecutive_errors": executor_health.get("consecutive_errors"),
        })

    progress_signal_candidates = [item for item in progress_signal_candidates if item.get("at")]
    progress_signal_candidates.sort(key=lambda item: item["at"])
    last_progress_signal_at = progress_signal_candidates[-1]["at"] if progress_signal_candidates else None

    artifact_paths = _dedupe_preserve(task.get("artifacts") or [])
    for item in task.get("downloads", []) or []:
        artifact = item.get("artifact")
        if nonempty(artifact) and artifact not in artifact_paths:
            artifact_paths.append(artifact)
    reporting = ensure_reporting(task)
    delivered_update_ids = [item.get("update_id") for item in reporting.get("delivered_updates", []) if item.get("update_id")]
    pending_update_ids = [item.get("update_id") for item in reporting.get("pending_updates", []) if item.get("update_id")]
    completed_steps = [item["id"] for item in workflow_states if item["state"] == "DONE"]
    remaining_steps = [item["id"] for item in workflow_states if item["state"] != "DONE"]
    if derived_status == "COMPLETED" and not remaining_steps:
        outcome_status = "SUCCESS"
    elif artifact_paths:
        outcome_status = "PARTIAL_SUCCESS" if remaining_steps or derived_status in {"BLOCKED", "FAILED"} else "SUCCESS"
    elif derived_status == "BLOCKED":
        outcome_status = "BLOCKED"
    elif derived_status == "FAILED":
    ...
    derived.update({
        ...
        "progress_signals": progress_signal_candidates[-12:],
        "out_of_bounds": any(item.startswith("serial_out_of_bounds:") for item in inconsistencies),
        "user_facing": {
            "status": derived_status,
            "truth_state": truth_state,
            "current_step": current_step,
            ...
            "delivered_update_ids": delivered_update_ids,
        },
    })

    # Keep top-level execution/control-plane projection converged with derived truth so
    # live diagnosis does not need to mentally reconstruct state from nested fields.
    task["status"] = derived_status
    if current_step is not None:
        task["current_checkpoint"] = current_step
    if last_progress_signal_at:
        task["last_checkpoint_at"] = last_progress_signal_at
    task["workflow_projection"] = workflow_states
    return task
...
def cmd_init(args):
    ...
    workflow = compile_validated_workflow(args.goal, args.workflow or [])
    ...
    task = {
        ...
        "heartbeat": {
            ...
            "progress_idle_threshold_sec": args.timeout_sec,
...
def cmd_block(args):
    ...
    if facts.get("failure_type"):
        ...
        # Increment transient-failure retry counter so monitor can enforce retry-first.
        step_key = f"{step_id}:{facts['failure_type']}"
        monitoring = task.setdefault("monitoring", {})
        current = int(monitoring.get("retry_count", {}).get(step_key, 0) or 0)
        monitoring.setdefault("retry_count", {})[step_key] = current + 1
    task["status"] = "BLOCKED"
